...
export const ScoreboardCard: React.FC<ScoreboardCardProps> = ({user, index}) => {
  return (
    <article className="gap-3 py-3 border-b border-slate-300 flex items-center w-full">
      <figure>
        <Image alt={`${user.username}-profile-picture`} src={user.picture} width={50} height={50} className="rounded-full selection:bg-transparent" />
      </figure>
